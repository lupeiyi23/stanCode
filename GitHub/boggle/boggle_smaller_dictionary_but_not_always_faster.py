...
def write_dictionary_excerpt(boggle, dictionary, dictionary_excerpt):
	...
	for key in boggle:
		...
		for letter in letters:
			if letter in letters_boggle:
				letters_boggle[letter] += 1
			else:
				letters_boggle[letter] = 1
	for word in dictionary:
		if 4 <= len(word) <= 16 and all_letters_fit(word, letters_boggle):
			dictionary_excerpt.append(word)


def all_letters_fit(s, letters_boggle):
	"""
	This function is to check if a word is spelled by only letters in boggle.
	:param s: (str) a word to check
	:param letters_boggle: a dictionary of all letters in boggle and their correspondent occurrence
	...
	for key in letters_s:
		val = letters_s[key]
		if key not in letters_boggle:
			return False
		if val > letters_boggle[key]:
			return False
	return True
